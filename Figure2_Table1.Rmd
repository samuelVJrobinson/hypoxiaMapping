...
fname <- paste0('../Dead_Zone_telecoupling/data/results_RF/err_rf_Day_DObottom_2014_2014.Rdata'); fname
load(fname)
...
err_3models <- rbind(llModErr %>% dplyr::mutate(mod = 'LLR'),
  ...
  ungroup() %>%
  arrange(mod, errType) %>%
  dplyr::mutate(mod = factor(mod, levels = c('LLR', 'RFR', 'FDA')),
...
ggsave('./figures/outOfSamp_comparison_3models_new.png', p, width=7, height=5)
```
