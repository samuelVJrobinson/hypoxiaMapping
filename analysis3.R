...
sDat <- sDat %>% select(-PC1:-PC4) %>% #Remove PCs from before
  mutate(numNA=apply(st_drop_geometry(sDat)[,3:16],1,function(x) sum(is.na(x)))) %>% #Count NAs in data columns
...
rm(sDat_pca,sDatMat_imputed) #cleanup
