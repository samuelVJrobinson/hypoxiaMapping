...
### 1.2 need to update the data --> the above data was based on data from `Img2Table_04_2021-03-18`, which extracted pixel values at 500m resolution, which is not correct. Need to use the most updated new data
...
```
  ...
  Per request by the RSE reviewer, we expanded the lags from 0-30 days to 0-60 days.
```{r - Loop 0-90 days, include=FALSE}
...
```{r link with DO - by day, include=FALSE}
...
interval <- 7  ## every  7 days as the time lag unit
interval <- 10 ## every 10 days as the time lag unit
# interval <-  5 ## every 10 days as the time lag unit
...
slots      <- ceiling(time_range/interval); slots ## round up the number

for (nw in seq(1, slots)) {
  # print(nw)
  ...
  ### write to .xlsx ---------------------------------
  fname <- paste0(dir.cleaned, 'by_timelag_withDO/sample_2000to2019_pixelValue_with', gsub('_', '', whichDO), '_',
                  nw, prefix, '.xlsx'); print(fname)
  writexl::write_xlsx(x = f, path = fname)
  ...
  ### write to .shp
  shp_name <- paste0('sample_2000to2019_pixelValue_with', gsub('_', '', whichDO), '_', 
                     nw, prefix, '_noNA', '.shp');  print(shp_name)
  shp_path <- paste0(dir.cleaned, 'by_timelag_withDO/shp/', shp_name); print(shp_path)
  st_write(obj = f_shp, dsn = shp_path, layer = shp_name, driver = "ESRI Shapefile", delete_dsn = T)
}
```
