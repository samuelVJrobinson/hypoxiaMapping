...
for (slice_id in slice_ls) {
  ...
  ## - write it as a geoTIFF
  fname <- paste0("./data/results/", which_dat, "map_", which_do, '_', date_ini, ".tif"); print(fname)
  writeRaster(r, filename = fname, overwrite=TRUE)
}
```
...
for (ras in ras_ls) {
  ...
  domap <-
    ...
    tm_layout(bg.color = bg_color, legend.height = 1,
...
func_domap_1col(n_plots = 9, h = 3.2/2, postfix = "_R1")
...
```{r eval=FALSE, include=FALSE}
## 2. in a 3*3 panel --------------------------
fname <- paste0("./figures/do_maps_", which_dat, '_', which_do,  "_3col_R1.png"); fname
